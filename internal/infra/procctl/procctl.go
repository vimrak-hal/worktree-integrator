// Package procctl は、server サブシステムの ProcessControl を実装する OS 依存の
// プロセス制御である。長時間稼働するサーバーを自身のセッションにデタッチして起動し
// （そのため CLI の終了やターミナルのクローズ後も生き残る）、停止時にはプロセスグループ
// 全体にシグナルを送る。生存確認と停止は pgid の数値ではなく proc.Ident（PID + 開始
// 時刻）で行い、シグナル送出前に必ず同一性を検証して、番号の再利用による無関係
// プロセスの誤殺を防ぐ。
//
// core/server がインターフェイス（ProcessControl）と状態機械を持ち、この infra
// パッケージがその唯一の本番実装を提供する。層の依存は core → infra の一方向であり、
// procctl は core を import しない。環境変数はドメイン語彙 wtenv.Pair ではなく、
// exec.Cmd.Env と同形の "KEY=VALUE" 文字列スライスで受け取る（合成は呼び出し側の
// 責務であり、この実装は与えられた環境をそのまま子プロセスへ渡す）。
package procctl
...
// 写すだけを担う。env は既に合成済みの環境（"KEY=VALUE" のスライス）であり、そのまま
// 子プロセスへ渡す。
func (u *UnixProcess) RunForeground(ctx context.Context, script, cwd string, env []string) (bool, error) {
