...
type tailer struct {
	...
	primed bool
	// dropping は初回ウィンドウの先頭断片（途中から始まる不完全な行）をまだ捨て切って
	// いない状態。末尾 64KB 内に改行が無ければ 1 回の poll で捨て切れないため、最初の
	// '\n' に到達するまで poll をまたいで捨て続ける不変条件を保つフィールドに昇格した。
	dropping bool
}
...
func (t *tailer) poll() ([]string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			// ファイルが（再）作成されたら初回扱いに戻す。primed も落とさないと、
			// 再作成されたファイルが initialWindow 超でも「先頭から全部読む」状態に
			// なり、末尾ウィンドウだけ読む不変条件が崩れる。捨て途中の断片状態
			// （dropping）も破棄する。
			t.offset = 0
			t.partial = nil
			t.primed = false
	...
	if size < t.offset {
		// ローテート／トランケート: 新しい内容を先頭から読み直す。先頭からなら最初の
		// 行は完全なので、捨て途中（dropping）ならここで解除する。
		t.offset = 0
		t.partial = nil
		t.dropping = false
	}
	...
	if t.dropping {
		if i := bytes.IndexByte(data, '\n'); i >= 0 {
			// 断片を捨て切った。以降は通常どおり行へ分解する。
			data = data[i+1:]
			t.dropping = false
		} else {
			// まだ改行に届かない。断片全体を保留し、次の poll でも捨て続ける。
			t.partial = data
			return nil, nil
		}
	}
