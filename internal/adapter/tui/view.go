...
func (m *model) View() string {
	...
	switch {
	case m.forms.form != nil:
		base = overlayCentered(base, m.forms.formDialog(m.dialogInnerW(), m.height), m.width)
	case m.doctor.doctorMode:
		base = overlayCentered(base, m.doctor.doctorDialog(m.doctorInnerW()), m.width)
	}
...
func (m *model) baseLines() []string {
	...
	rightBox := m.renderBox(m.log.logTitle(), m.log.rightLines(), rightInner, innerH)
...
func (m *model) leftColumn(innerW, innerH int) []string {
	...
	treeBox := m.renderBox(m.leftTitle(), m.tree.visibleNodeLines(treeInnerH), innerW, treeInnerH)
	eventBox := m.renderBox(m.eventTitle(), m.op.eventLines(), innerW, eventBoxInnerH)
...
func (m *model) treeOnlyLines(h int) []string {
	status := m.op.statusLine()
	if status == "" {
		return m.tree.visibleNodeLines(h)
	}
	return append(m.tree.visibleNodeLines(max(1, h-1)), status)
}
